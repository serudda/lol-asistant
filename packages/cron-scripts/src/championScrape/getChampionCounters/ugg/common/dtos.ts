import { getChampionSlugForSource, Sources } from '@lol-assistant/common';
import { normalizeStringToSlug, U_GG_CHAMPION_URL, type UGGRank } from '../../../../common';
import type { SourceCounter } from '../../common/types';
import type { UGGCounterOptionsData } from './types';

export const uggChampCounterDto = (item: UGGCounterOptionsData, rankTier: UGGRank, id: number): SourceCounter => {
  // Some champions have different slugs depending on the data
  const sourceChampionSlug = getChampionSlugForSource(normalizeStringToSlug(item.champion), Sources.U_GG);

  return {
    rank: id,
    sourceChampionSlug,
    sourceRankTier: rankTier,
    champUrl: `${U_GG_CHAMPION_URL}/${sourceChampionSlug}/build/${item.role.toLowerCase()}?rank=${rankTier}`,
    role: item.role,
    source: Sources.U_GG,
    counterWinRate: parseFloat(item.counterWinRate),
    matches: item.matches,
  };
};
